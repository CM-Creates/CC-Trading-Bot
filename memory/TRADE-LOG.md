...
### Jul 29 — EOD Snapshot (Day 28, Wednesday) — BLOCKED (Alpaca auth 401)
**Portfolio:** unavailable | **Cash:** unavailable | **Day P&L:** uncomputable | **Phase P&L:** uncomputable

| Ticker | Shares | Entry   | Last known close | Stop (live GTC) |
|--------|--------|---------|------------------|-----------------|
| PG     | 99     | $150.72 | $148.63 (Jul 27) | $138.89         |
| KO     | 35     | $83.10  | $84.09  (Jul 27) | $77.11          |

**Notes:** Daily summary could NOT complete — `python tools/alpaca.py preflight` returned exit 4 (terminal auth): Alpaca reached, credentials rejected (HTTP 401, "unauthorized"). Key fingerprint present (ALPACA_API_KEY prefix PK, len 26; secret len 44; no whitespace), so the env vars are populated but stale/expired/revoked. No account, positions, or orders could be pulled this run, so today's equity, Day P&L, and Phase P&L are uncomputable. Last verified equity was $99,827.72 (Jul 27 EOD); note the Jul 28 snapshot is also absent from this log, so continuity has a two-session gap. Both positions (PG 99 sh @ $150.72, KO 35 sh @ $83.10) are assumed still held with their 10% trailing stops live on Alpaca (PG $138.89 / HWM $154.32; KO $77.11 / HWM $85.68) — but this could NOT be confirmed against the broker this run. Per Blocker Protocol: not retryable, no keys fabricated, no .env written, TLS untouched; one email alert sent naming the exact fix. FIX (owner action): rotate ALPACA_API_KEY + ALPACA_SECRET_KEY in the web ENVIRONMENT CONFIG (process env vars, NOT a repo .env), then re-run the routine. Until keys are refreshed, pre-market / market-open / midday runs will all fail the same way and stops cannot be verified or adjusted.
